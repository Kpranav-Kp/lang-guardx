"""
lang_guardx/langchain_layer/core.py

Layer 2 — Single Entry Point
------------------------------
LangGuardXLayer2 is the unified interface for all of Layer 2.
It is the only class other layers and end users need to import
from this package for protection.

PromptGuard parallel:
  PromptGuard's Layer 2 is purely structural prompt formatting
  (ChatML/JSON role separation). LangGuardXLayer2 inherits that
  and wraps it with SQL policy enforcement, tool policy, and
  trace collection — making it the single coordinator for all
  agent-level and chain-level security decisions.

Relationship to other files in this directory:
  sql_policy.py  — SQLPolicy config + SQLPolicyEngine (validate + rewrite)
  chain.py       — ProtectedChain, protect_chain() factory
  agent.py       — ProtectedAgent, protect_agent() factory, AgentTrace
  core.py        — THIS FILE: LangGuardXLayer2 ties them all together

Usage (simple):
    from lang_guardx.langchain_layer.core import LangGuardXLayer2

    layer2 = LangGuardXLayer2.for_chain(
        chain=my_sql_chain,
        permitted_tables=["products", "reviews"],
        restricted_columns={"users": ["password", "email"]},
...
Usage (agent):
    layer2 = LangGuardXLayer2.for_agent(
        agent_executor=my_agent,
        permitted_tables=["products", "orders"],
        require_user_scope=True,
        current_user_id=user.id,
        system_prompt="You are an order assistant.",
    )
    response, trace = layer2.run("what did I order last month")

Usage (custom policy):
    from lang_guardx.langchain_layer.sql_policy import SQLPolicy
